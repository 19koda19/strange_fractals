...
const outputDirectory = process.argv[3] || '/private/tmp/noema-octave-boundary';
const phrase = (process.argv.slice(4).join(' ') || 'octave boundary remembers violet gravity')
  .replace(/\s+/gu, ' ')
  .trim();
...
function viewSnapshot(renderer) {
  const bandView = renderer?.portalBandViews?.find(([epoch]) => epoch === renderer.zoomEpoch)?.[1];
  // A live portal can drift a fraction as the phrase morph and neural wave
  // settle. Compare the reversible band origin, not the camera correction
  // that deliberately follows that moving geometry.
  const stableCamera = bandView?.length >= 4 ? bandView.slice(2, 4) : renderer?.cameraOffset;
  return [...(renderer?.focus || []), ...(stableCamera || [])];
}
...
function makeDepthPlan(bandOctaves) {
  const before = (multiple) => bandOctaves * multiple - boundaryOffset;
  const after = (multiple) => bandOctaves * multiple + boundaryOffset;
  return [
    { leg: 'positive-in', targetDepth: 0, boundary: 0, side: 'baseline' },
    { leg: 'positive-in', targetDepth: before(1), boundary: bandOctaves, side: 'before' },
    { leg: 'positive-in', targetDepth: after(1), boundary: bandOctaves, side: 'after' },
    { leg: 'positive-in', targetDepth: before(2), boundary: bandOctaves * 2, side: 'before' },
    { leg: 'positive-in', targetDepth: after(2), boundary: bandOctaves * 2, side: 'after' },
    { leg: 'positive-return', targetDepth: before(2), boundary: bandOctaves * 2, side: 'before' },
    { leg: 'positive-return', targetDepth: after(1), boundary: bandOctaves, side: 'after' },
    { leg: 'positive-return', targetDepth: before(1), boundary: bandOctaves, side: 'before' },
    { leg: 'positive-round-trip', targetDepth: 0, boundary: 0, side: 'baseline' },
    { leg: 'negative-out', targetDepth: -before(1), boundary: -bandOctaves, side: 'before' },
    { leg: 'negative-out', targetDepth: -after(1), boundary: -bandOctaves, side: 'after' },
    { leg: 'negative-out', targetDepth: -before(2), boundary: -bandOctaves * 2, side: 'before' },
    { leg: 'negative-out', targetDepth: -after(2), boundary: -bandOctaves * 2, side: 'after' },
  ].map((stop) => ({
    ...stop,
...
function inspectRendererState(renderer, stop, expectedHistoryKeys, bandOctaves) {
  ...
  if (Math.abs(renderer.bandOctaves - bandOctaves) > 1e-9) issues.push('zoom band size changed during capture');
...
try {
  ...
  baseline = await evaluate(`(() => {
    ...
    async function zoomTo(targetDepth) {
      ...
      if (!canvas || !renderer) throw new Error('No development renderer is available for boundary capture.');
      const bounds = canvas.getBoundingClientRect();
      let remaining = targetDepth - renderer.logZoom;
      let dispatchCount = 0;

      while (Math.abs(remaining) > 0.00001) {
        const change = Math.sign(remaining) * Math.min(0.1, Math.abs(remaining));
        const event = new WheelEvent('wheel', {
          deltaY: -change / 0.0024,
        ...
        if (dispatchCount > 1000) throw new Error('Paced zoom failed to converge.');
      }
    ...
    Object.defineProperty(window, '__noemaOctaveBoundaryGuard', {
      configurable: true,
      enumerable: false,
      writable: false,
      value: Object.freeze({ inspect, zoomTo, release }),
    });
    return inspect();
  })()`);
  captureLockInstalled = Boolean(baseline?.locked);
  const baselineStability = inspectFixture(baseline);
  baseline = { state: baseline, stability: baselineStability };
  if (!baselineStability.stable) contamination.push({ stage: 'baseline', issues: baselineStability.issues });

  // Allow the deterministic semantic enhancement and birth transition to settle
  // before inspecting the scale-band handoffs.
  ...
  if (rendererConfiguration?.error) {
    diagnostics.push({ type: 'renderer', details: rendererConfiguration.error });
  }
  ...
  for (let index = 0; index < depthPlan.length; index += 1) {
    ...
    const zoomDispatch = await evaluate(
      `window.__noemaOctaveBoundaryGuard.zoomTo(${JSON.stringify(targetDepth)})`,
      true,
    );
    await new Promise((resolve) => setTimeout(resolve, targetDepth === 0 ? 220 : 520));
    ...
    const timing = await evaluate(`new Promise((resolve) => {
      ...
      function frame(now) {
        ...
        previous = now;
        if (samples.length < 25) requestAnimationFrame(frame);
        else {
          samples.shift();
    ...
    const inspection = await evaluate(`(() => {
      ...
      const epoch = zoomPosition?.epoch ?? null;
      const phase = zoomPosition?.phase ?? null;
      return {
        ...
        captureState: window.__noemaOctaveBoundaryGuard?.inspect() ?? null,
      };
    })()`);

    const stability = inspectFixture(inspection.captureState);
    stability.issues.push(
      ...inspectRendererState(inspection.renderer, stop, expectedHistoryKeys, bandOctaves),
    );
    if (inspection.canvasCount !== 1) stability.issues.push(`expected one canvas; found ${inspection.canvasCount}`);
    if (stop.leg === 'positive-round-trip' && expectedHistoryKeys.size !== 0) {
      stability.issues.push('positive round trip did not consume its expected navigation history');
    }
    const viewKey = targetDepth.toFixed(6);
    ...
    if (firstView) {
      ...
      if (viewDelta > 0.00002) {
        stability.issues.push(`repeated depth ${targetDepth} drifted by ${viewDelta}`);
      }
    } else {
      firstViewsAtDepth.set(viewKey, currentView);
    }
    stability.stable = stability.issues.length === 0;
    if (!stability.stable) {
      contamination.push({
    ...
    const name = `boundary-${String(index).padStart(2, '0')}-depth-${depthSlug(targetDepth)}.png`;
  ...
  finalGuard = await evaluate('window.__noemaOctaveBoundaryGuard?.inspect() ?? null');
  finalReset = await evaluate(`(() => {
    const renderer = window.__noemaDev?.renderer;
  ...
  if (captureLockInstalled) {
    try {
      guardRelease = await evaluate('window.__noemaOctaveBoundaryGuard?.release() ?? null');
    } catch (error) {
...
const summary = {
  ...
  boundaryOffset,
  depthPlan,
  pacing: { maximumOctavesPerFrame: 0.1, anchor: { x: 0.5, y: 0.5 } },
  baseline,
  frames,
  finalGuard,
  finalReset,
